...
int main(int argc, char** argv){ 
	...
	// Recorremos ambas matrices por filas y usamos desenrollado de bucles.
	for (int f = 0; f < N; f++)
        	for (int c= 0; c < N; c++) {
            		for (int inner = 0; inner < N; inner+=4) {
                		M3[f][inner] += M1[f][c] * M2[c][inner];
                		M3[f][inner+1] += M1[f][c] * M2[c][inner+1];
                		M3[f][inner+2] += M1[f][c] * M2[c][inner+2];
                		M3[f][inner+3] += M1[f][c] * M2[c][inner+3];
            		}
        	}
